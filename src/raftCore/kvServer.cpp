...
// get和put//append执行的具体细节不一样的
// PutAppend在收到raft消息之后执行，在具体函数中仅判断幂等性（是否重复）
// get函數收到raft消息之后在，因为get无论是否重复都可以再执行
void KvServer::PutAppend(const raftKVRpcProtoc::PutAppendArgs *args, raftKVRpcProtoc::PutAppendReply *reply)
{
  ...
  if (!isleader)
  {
    DPrintf("[func -KvServer::PutAppend -kvserver{%d}]From Client %s (Request %d) To Server %d, key %s, raftIndex %d , but not leader",
  ...
  DPrintf("[func -KvServer::PutAppend -kvserver{%d}]From Client %s (Request %d) To Server %d, key %s, raftIndex %d , is leader ",
          m_me, args->clientid().c_str(), args->requestid(), m_me, op.Key.c_str(), raftIndex);
  ...
  if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp))
  {
    DPrintf("[func -KvServer::PutAppend -kvserver{%d}]TIMEOUT PUTAPPEND !!!! Server %d , get Command <-- Index:%d , "
            ...
            m_me, m_me, raftIndex, op.ClientId.c_str(), op.RequestId, op.Operation.c_str(), op.Key.c_str(), op.Value.c_str());
...
void KvServer::ReadRaftApplyCommandLoop()
{
  while (true)
  {
    ...
    // 监听其 Raft 应用的每个命令，交付给相关的 RPC 处理程序
...
bool KvServer::SendMessageToWaitChan(const Op &op, int raftIndex)
{
  ...
  DPrintf("[RaftApplyMessageSendToWaitChan--> raftserver{%d}] , Send Command --> Index:{%d} , ClientId {%s}, RequestId "
    ...
    : m_skipList(6)
{
  ...
  // 这里使用睡眠来保证
  std::cout << "raftServer node:" << m_me << " start to sleep to wait all other raftnods start!!!" << std::endl;
  sleep(6);
